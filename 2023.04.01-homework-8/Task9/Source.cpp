...
public:
	...
	void getMatrixFromAdjacencyList(int vertexes);
...
private:
	...
	///??????? ??????? ??????? ? ?????? ??????? _edges
	void initEdges();
	int VertexEdgesCount(int vertex);
...
int main(int argc, char* argv[])
{
	...
	int n = 0;
	std::cin >> n;
	graph.getMatrixFromAdjacencyList(n);
	std::cout << n << std::endl;
	graph.PrintMatrix();
...
void CGraph::getMatrixFromAdjacencyList(int vertexes)
{
	int c = 0;
	int temp = 0;
	_vertexes = vertexes;
...
int CGraph::VertexEdgesCount(int vertex)
{
	int itog = 0;
	for (int i = 0; i < _vertexes; ++i)
	{
		itog = itog + _matrix[vertex][i];
	}
	return itog;
}
