...
public:
	...
	int BadBridgesCount(int* HillColors);
...
int main(int argc, char* argv[])
{
	...
	for (int i = 0; i < n; ++i)
	{
		std::cin >> HillColors[i];
	}
	std::cout << graph.BadBridgesCount(HillColors);
...
int CGraph::BadBridgesCount(int* HillColors)
{
	int itog = 0;
	for (int i = 0; i < _vertexes; ++i)
	{
		...
		{
			if ((_matrix[i][j] != 0) && (HillColors[i] != HillColors[j]))
			{
				itog++;
			}
		}
