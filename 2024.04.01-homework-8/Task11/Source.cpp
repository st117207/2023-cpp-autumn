...
bool CGraph::IsGraphRegular()
{
	int temp = 0;
	temp = PowerVertex(1);
	for (int i = 2; i < _vertexes + 1; ++i)
	{
		if (PowerVertex(i) != temp)
		{
			return false;
		}
	}
...
int CGraph::PowerVertex(int vertex)
{
	int itog = 0;
	for (int i = 0; i < _vertexes; ++i)
	{
		itog = itog + _matrix[vertex - 1][i] + _matrix[i][vertex - 1];
	}
	return itog / 2;
}
